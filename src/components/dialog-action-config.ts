    ...
    return html`
      <span class="header_title"
        >${localize(
          ...
          this.hass.language,
          '{action}',
          this.hass!.localize(`ui.card.alarm_control_panel.${this._params!.action}`)
        )}</span
      >
      <ha-icon-button
    ...
    return html`
      ...
      >
        <paper-input
          ...
          @change=${(ev: Event) =>
            this._updateConfig({ button_label: String((ev.target as HTMLInputElement).value).trim() })}
        ></paper-input>
        ...
        <paper-input
          ...
          placeholder="${this.hass.localize(
            `component.alarm_control_panel.state._.${ActionToState[this._params!.action]}`
          )}"
          @change=${(ev: Event) =>
            this._updateConfig({ state_label: String((ev.target as HTMLInputElement).value).trim() })}
        ></paper-input>
